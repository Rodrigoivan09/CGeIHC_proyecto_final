...
Toroide::Toroide(float radioCentro, float radioT, int slices, int stacks) :
	radioCentro(radioCentro), radioT(radioT), slices(slices), stacks(stacks), VAO(0), VBO(0), EBO(0) {
}
...
void Toroide::init() {
	...
	for (int i = 0; i <= stacks; ++i) {
		...
		for (int j = 0; j <= slices; ++j) {
			...
			vertexC[i * (slices + 1) + j].position = radioCentro * glm::vec3(X, Y, Z);
